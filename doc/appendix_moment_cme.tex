...
\eref{eq_cme_matrix} we have defined the following matrices:
...
To prove this we factorize \eref{eq_mom_pde} as
\begin{equation}
  \begin{aligned}
    \dt{\bb{\ee{m^x p^y}}} &=
    \Km \bb{\ee{m^x p^y}}\\
...
therefore $y' \leq y$, and the highest moment in \eref{eq_second_mom_mRNA} also
...
$x$ and $y$ we now simplify \eref{eq_mom_pde_factorized} making use of the
binomial expansion
\begin{equation}
