...
\eref{eq_sum_mom} contains the definition of a moment from \eref{eq_mom_def}.
That means that we can rewrite it as
\begin{equation}
  \dt{}\left[ \smp m^x p^y \PP(m, p) \right] = \dt{\bb{\ee{m^x p^y}}}.
\end{equation}

Distributing the sum on the right-hand
side of \eref{eq_cme_matrix} gives
\begin{equation}
  \begin{aligned}
    \dt{\bb{\ee{m^x p^y}}} &=
...
\eref{eq_master_sum} involving $\PP(m, p)$ we can again use \eref{eq_mom_def}
...
For the sums terms in \eref{eq_master_sum} involving $\PP(m \pm 1, p)$ or
...
name is somehow arbitrary, and the right-hand side of \eref{eq_reindex} can be
written as
\begin{equation}
  \Rm \sum_{m'=0}^\infty \sum_p (m' + 1)^x p^y \PP(m', p) =
  \Rm \bb{\ee{(m+1)^x p^y}},
...
this is useful since our definition of a moment from \eref{eq_mom_def} requires
...
A very interesting and useful feature of \eref{eq_mom_pde} is that for a given
...
To prove this we rewrite \eref{eq_mom_pde} as
\begin{equation}
  \begin{aligned}
    \dt{\bb{\ee{m^x p^y}}} &=
    \Km \bb{\ee{m^x p^y}}\\
...
\eref{eq_mom_pde_factorized} then becomes
\begin{equation}
  \begin{aligned}
    \dt{\bb{\ee{m^2 p^0}}} &=
    \Km \bb{\ee{m^2 p^0}}\\
...
\eref{eq_second_mom_mRNA} satisfies both of our conditions. Since we set $y$ to
...
\eref{eq_second_mom_mRNA}  satisfies $x' + y' \leq x + y$ since the second
...
\eref{eq_mom_pde_factorized} making use of the binomial expansion
\begin{equation}
...
which again satisfies the conditions on \eref{eq_mom_conditions}.
...
on lower moments that satisfy \eref{eq_mom_conditions}.
