...
in \mrm{ref model section} present a had mathematical challenge. As presented in
\mrm{ref FISH  mcmc section} Peccoud \& Ycart derived a closed-form solution for
...
moments. Mathematically what this means is that $P_H(x)$ must satisfy
\begin{equation}
  \sum_x x^i P_H(x) =
  \sum_x {x^i \over \mathcal{Z}}
  \exp \left( - \sum_{i=1}^m \lambda_i x^i \right) = \ee{x^i}.
\end{equation}

As an example of how to apply the MaxEnt principle let us use the classic
example of a six-face die. If we are only told that after a large number of die
...
\end{equation}
Using the Python maxentropy package we can easily find the numerical value of
the Lagrange multiplier $\lambda$  that satisfies our constraint.
...
Having explained the theoretical background behind the MaxEnt approximation we
now use it to predict gene expression distributions

\begin{figure}[h!]
	\centering \includegraphics
  {../fig/MaxEnt_approx_joint/CDF_grid.pdf}
	\caption{\textbf{Maximum entropy cumulative distributions for simple
  repression constructs.} Cumulative distribution functions for different
  biophysical parameters. From left to right the repressor-DNA affinity
  decreases as defined by the three lacI operators O1 ($-15.3 \; k_BT$), O2 ($
  -13.9 \; k_BT$), and O3 ($-9.7 \; k_BT$). From top to bottom the mean
  repressor copy number per cell increases. The twelve curves on each plot
  represent the twelve IPTG concentrations used in the experiments with higher
  concentrations represented by lighter colors (See \mrm{ref methods} for
