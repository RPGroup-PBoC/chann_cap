...
in \secref{sec_model} represent a hard mathematical challenge. As presented
in \siref{supp_param_inference} Peccoud and Ycart derived a closed-form solution
...
Let the $M \times N$ matrix $\bb{A}$ contain all the factors used to compute the
...
along the cell cycle (See \siref{supp_multi_gene}).
...
\begin{figure}[h!]
	\centering \includegraphics
  {../fig/MaxEnt_approx_joint/PMF_grid_joyplot_mRNA_nonpoisson.pdf}
	...
	The curves on each plot represent different IPTG concentrations.}
  \label{fig_pmf_mRNA}
\end{figure}

\begin{figure}[h!]
	\centering \includegraphics
  {../fig/MaxEnt_approx_joint/PMF_grid_joyplot_protein_nonpoisson.pdf}
...
around this issue we recall that the fold-change in gene expression that we
defined as the ratio of the gene expression level in the presence of the
repressor and the expression level of a knockout strain is a non-dimensional
quantity. Therefore we normalize all of our single-cell measurements by the mean
fluorescence value of the $\Delta lacI$ strain with the proper background
...
level $\ee{p}$, having now non-dimensional scales that can be directly compared.
\fref{sfig_cdf_O1}, \fref{sfig_cdf_O2}, and \fref{sfig_cdf_O3} show comparison
of the cumulative distribution functions for different repressor binding sites
(different figures), repressor copy numbers (rows), and inducer concentrations
(columns).

\begin{figure}[h!]
	\centering \includegraphics
  {../fig/MaxEnt_approx_joint/ECDF_O1_theory_experiment_nonpoisson.pdf}
	\caption{\textbf{Experiment vs. theory comparison for operator O1.}
  Example fold-change empirical cumulative distribution functions (ECDF) for
  strains with the O1 operator ($\eR = -15.3 \; k_BT$) as a function of
  repressor copy numbers (rows) and inducer concentrations (columns). The color
  curves represent single-cell microscopy measurements while the dashed black
  lines represent the theoretical distributions as reconstructed by the maximum
  entropy principle.}
  \label{sfig_cdf_O1}
\end{figure}
...
\begin{figure}[h!]
	\centering \includegraphics
  {../fig/MaxEnt_approx_joint/ECDF_O2_theory_experiment_nonpoisson.pdf}
  \caption{\textbf{Experiment vs. theory comparison for operator O2.}
  Example fold-change empirical cumulative distribution functions (ECDF) for
  strains with the O2 operator ($\eR = -13.9 \; k_BT$) as a function of
  repressor copy numbers (rows) and inducer concentrations (columns). The color
  curves represent single-cell microscopy measurements while the dashed black
  lines represent the theoretical distributions as reconstructed by the maximum
  entropy principle.}
...
\begin{figure}[h!]
	\centering \includegraphics
  {../fig/MaxEnt_approx_joint/ECDF_O3_theory_experiment_nonpoisson.pdf}
	\caption{\textbf{Experiment vs. theory comparison for operator O3.}
  Example fold-change empirical cumulative distribution functions (ECDF) for
  strains with the O3 operator ($\eR = -9.7 \; k_BT$) as a function of
  repressor copy numbers (rows) and inducer concentrations (columns). The color
  curves represent single-cell microscopy measurements while the dashed black
  lines represent the theoretical distributions as reconstructed by the maximum
  entropy principle.}
