...
\fref{fig3_cell_cycle}(B) upper panel). On the other hand the mean protein level
...
cycle to cycle follows a repetitive pattern (See \fref{fig3_cell_cycle}(B) lower
panel).
...
done by applying an averaging like the one in \eref{eq_time_avg} for the span
of a cell cycle.
...
non-dimensional \mrm{adimensional?} quantity defined as the fold-change in gene
...
\end{equation}
where $\ee{p(R \neq 0)}_c$ represents the mean protein count for a cell with
...
analysis appendix for further details}. The lower row shows the noise in gene
expression $\eta$ defined as the standard deviation over the mean. The good
correspondence between the theoretical predictions and the experimental data
could only be achieved when considering the gene copy number variability treated
in this section \mrm{See Appendix XX for comparison with single-copy
predictions}
