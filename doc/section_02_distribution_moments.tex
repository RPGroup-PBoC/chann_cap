...
area of research. An alternative approach is to find schemes to approximate the
distribution. One such scheme, the maximum entropy principle, makes use of the
...
\eref{eq_gral_mom} has no moment-closure problem \cite{Voliotis2014a}. What
this means is that the dynamical equation for a given moment only depends on
lower moments (See \siref{supp_moments} for full proof). This feature of our
...
containing all moments up to $\bb{\ee{m^x p^y}}$ for all promoter states. This
is
\begin{equation}
...
moments of interest as cells progress through the cell cycle. This represents
an improvement compared to trying to evolve the linear although infinite set of
equations that describe $\bb{P}(m, p)$.
