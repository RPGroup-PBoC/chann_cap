...
\begin{figure}[h!]
  ...
  {../fig/chemical_master_mRNA_FISH/lacUV5_mRNA_double_expo_corner_plot.pdf}
	...
	the mRNA degradation rate $\gm$) $\kpon = 6.8^{+1}_{-0.5}$, $\kpoff =
	112^{+440}_{-50}$ and $r_m = 203^{+698}_{-82}$ which are the modes of
...
for the parameters: $\kpon = {0.03}_{-0.003}^{+0.005} s^{-1}$, $\kpoff = {0.6}
_{-0.2}^{+2.4} s^{-1}$, and $r_m = {1}_{-0.4}^{+3} s^{-1}$. \mrm{again need to
compare with what is known about these rates.}.
...
form
\begin{equation}
  \ee{m (R \neq 0)} = (2 - \phi){r_m \over \gm} {\kroff\kron
...
the kinetic model presented in \fref{fig2_minimal_model}(A) this time can be
