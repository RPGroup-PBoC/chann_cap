...
signals as depicted in \fref{fig1_intro}(A) for a bacterial cell sensing a
chemical concentration. At the molecular level where signal transduction unfolds
...
mapping between inputs and outputs. In that sense cells perform a Bayesian
inference of the state of the environment given their phenotypic response as
schematized in \fref{fig1_intro}(B). The question then becomes how to analyze
...
circuits process. The question lays at the heart of understanding the precision
of the cellular response to environmental signals. \fref{fig1_intro}(C)
schematizes a scenario in which for three different stimulus (inducer
concentrations) two bacterial strains respond with different levels of precision
given the degrees of overlap between outputs. It is precisely this overlap
between responses that determines the resolution with which cells can
...
\begin{figure}[h!]
  ...
  {./fig/main/intro_v02.pdf}
	...
	signaling systems can be modeled as noisy communication channels. (B) As cells
	respond to external stimulus they perform a Bayesian inference of the state of
	the environment. As the phenotype (protein level) serves as the internal
	...
	green bacteria responds more precisely than the purple bacteria since the
	output distributions overlap less (middle panel). This allows the green strain
	to make a more precise inference of the environmental state given a phenotypic
	response.}
  \label{fig1_intro}
\end{figure}
