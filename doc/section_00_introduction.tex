...
thought as a generalize correlation coefficient that quantifies how much does
the uncertainty about a random event decreases when we get to know the outcome
of another random event on average \cite{Kinney2010}.
...
detailed models with precision measurements on a simple system tractable both
theoretically and experimentally in which molecular parameters can be varied in
a controlled manner.
\mrm{This is a long sentence. Not sure how to break it into two.}

Over the last decade the dialogue between theory and experiments in gene
regulation has led to predictive power of models not only over the mean, but
the noise in gene expression as a function of relevant parameters such as
...
much information genetic circuits can process. The question lays at the heart
...
imaging system the point spread function limits the ability to resolve two
light emitting point sources.
...
\eref{eq_mutual_info} maximized over all possible input distributions $P(c)$.
...
\sm{This need more justification because it is unlikely that E coli can shape
lactose concentration of environment. More likely that we can claim this is
related to Cramer-Rao bound, thus to Wingreen PRL bounds on chemical sensory
processing.}
...
\begin{figure}[h!]
  ...
  {./fig/main/fig1_intro.pdf}
	...
	(B) We treat cellular response to an external stimuli as a Bayesian
