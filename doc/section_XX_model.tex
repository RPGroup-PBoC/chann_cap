...
\fref{fig_cell_area} shows the separation of these two groups based on area.
...
from equilibrium models \cite{Phillips2015}. Garcia \& Phillips
\cite{Garcia2011c} defined the fold-change in gene expression as
\begin{equation}
  \foldchange = {\text{gene expression}(R > 0) \over
                 \text{gene expression}(R = 0)},
\end{equation}
where $R$ is the number of transcriptional repressors per cell. Basically the
fold-change is the expression level in the presence of the repressor divided by
the expression level in the absence of regulation. In the language of
statistical mechanics this expression is of the form \cite{Garcia2011c}
\begin{equation}
