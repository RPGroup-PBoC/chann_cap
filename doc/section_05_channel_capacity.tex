...
-- effectively the distributions shown in \fref{fig4_maxent}. The channel
...
Although our theoretical predictions in \fref{fig5_channcap}(A) do not
...
curve in \fref{fig5_channcap}(A)) there is a rapid increment in the channel
capacity, but the maximum value reached is smaller compared to a weaker binding
site (orange curve in \fref{fig5_channcap}(A)). 
\sm{Can you show that the systematic multiplicative error in moments of p(g|c)
leads to a systematic additive error in C, maybe if only for small noise
approximation?  I think this might be done very simply, if I remember the
equation for C in the small noise approximation correctly.}
...
\begin{figure}[h!]
  ...
  {./fig/main/fig5_channcap.pdf}
	...
	Solid lines indicate plot in logarithmic scale, while dashed line indicates
	linear scale ton include zero. (B) Example input-output functions of
	opposite limits of channel capacity. Lower panel illustrates that zero
	channel capacity indicates that all distributions overlap. Upper panel
