...
derived in \mrm{reference moment derivation section} for a time $t = [0, t_s]$
...
\mrm{These are rather technical details. But I'm just dumping all of what I did
without thinking where will this end.}
\begin{enumerate}
...
This is expected since the parameters for the mRNA production were determined in
the first place under this assumption (See \mrm{cite mRNA MCMC section}). On the
other hand given that this relaxation time is determined by the degradation rate
the protein doesn't reach such steady state for either promoter state.
Interestingly once a couple of cell cycles have passed the first moment seems to
have a reproducible trajectory over cell cycles. We call this a dynamic
steady-state for the protein.

\begin{figure}[h!]
	\centering \includegraphics
  {../fig/moment_dynamics_numeric/first_mom_cycles.pdf}
	\caption{\textbf{First moment dynamics over cell the cell cycle.}(A) mean
	mRNA and (B) mean protein copy number as the cell cycle progresses. The dark
	shaded region delimits the fraction of the cell cycle that cells spend with  a
	single copy of the promoter. The light shaded region delimits the fraction of
...
first moment is computed as the integral defined in \eref{eq_moment_avg}.
