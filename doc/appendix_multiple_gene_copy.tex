...
\begin{enumerate}
  ...
  up to one. Initialize the rest of the moments with zeros.
...
asynchronous culture, cells are sampled from any time point along their
...
along the cell cycle in order to compute these averages.
...
moments). As mentioned before, in order to calculate the first moment along the
...
therefore the repressor on rate $\kron$ remains constant along the cell cycle.
...
\begin{figure}[h!]
  ...
  {../fig/moment_dynamics_numeric/IPTG_titration.pdf}
	...
	\cite{Razo-Mejia2018} with repressor-DNA binding energy $\eR = -13.5$ while
	the kinetic model is computed by numerically integrating the moment dynamics
	over several cell cycles, and then averaging over the extend of a single cell
  cycle as defined in \eref{seq_moment_avg}.}
...
standard deviation divided by the mean, is a dimensionless quantification of how
much variability there is with respect to the mean of a distribution. As we will
show below this quantity has the advantage compared to the also commonly used
...
(See \fref{sfig_noise_comparison}), we can asses whether or not this model is
...
fluorescent arbitrary units (a.u.) the noise is a dimensionless quantity. To see
...
number $p$. Substituting this definition on \eref{seq_noise_protein} gives
\begin{equation}
