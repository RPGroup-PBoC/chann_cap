...
mutual information should get closer to the actual one.
...
of the mutual information is valid. This suggests that the empirical choice for
the expansion of the mutual information is a good approximation. To test this
idea we repeated the bootstrap estimate of \fref{sfig_channcap_bootstrap}
sampling 10\%, 20\%, and so on until taking 100\% of the data. We repeated this
for different number of bins since a priori for arbitrary units of fluorescence
we do not have a way to select the optimal number of bins.
\fref{sfig_channcap_lin_reg} shows the result of these estimates. We can see
that the linear relationship proposed in \eref{seq_mutual_biased} holds true for
