...
hinted in \secref{sec_moments} the maximum entropy principle, first proposed by
...
a probability distribution $P_H$ of the form (See \siref{supp_maxent} for full
derivation)
\begin{equation}
...
any difference in the concentration of the signal. As the number of repressors
is increased, the degree of overlap between distributions decreases, allowing
cells to better resolve the value of the signal input. On the opposite extreme
the lower left panel shows a strong binding site ($\eR = -15.3 \; k_BT$) and a
...
repression skews all distributions towards lower copy numbers, giving again
little ability for the cells to resolve the inputs. In \siref{supp_maxent} we
show the comparison of these predicted distributions with the experimental
single-cell fluorescence distributions. In the following section we formalize
...
\begin{figure}[h!]
  ...
  {./fig/main/fig4_maxent.pdf}
	...
	protein distribution used to constrain the maximum entropy approximation were
	computed by integrating \eref{eq_gral_mom} as cells progressed through the
	cell cycle as described in \secref{sec_cell_cycle}.}
  \label{fig4_maxent}
\end{figure}
