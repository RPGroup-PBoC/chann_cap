...
single-cell fluorescence distributions. The right column compares predictions
as obtained from the zero-parameter fit. As expected the theoretical
distributions (dashed lines) underestimate the width of the experimental data.
The left column shows the predicted distributions when a single multiplicative
factor for all distributions is fit to the data (See \siref{supp_multi_gene}
for details), improving the agreement of the predictions. In the following
...
\begin{figure}[h!]
  ...
  {./fig/main/fig4_maxent.pdf}
	...
	(dashed lines). Left column shows zero-parameter fit predictions. Right
	column shows predictions with a multiplicative factor on the variance of
	the distribution fit to the data. Distributions were normalized to the mean
	expression value of the unregulated strain in order to compare theoretical
	predictions in discrete protein counts with experimental fluorescent
