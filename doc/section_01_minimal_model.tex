...
distribution of gene expression with parameters such as repressor copy number
and its affinity for the DNA are systematically varied.
...
uncover the physics of this genetic circuit \mrm{Aztec pyramid ref}. In doing so
...
changes of the repressor and the slower downstream such as the open-complex
formation processes allow us to model the probability of the repressor being in
the active state as an equilibrium MWC process. The parameters of the MWC model
$K_A$, $K_I$ and $\eAI$ were previously characterized from video-microscopy and
